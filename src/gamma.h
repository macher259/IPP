 ...
 * @author Maciej Herdon <mh418267@mimuw.edu.pl>
 ...
 * ruchu i jest przynajmniej jedno pole zajęte przez innego gracza oraz, czy
 * wykonanie tego złotego ruchu nie spowoduje przekroczenia dozwolonej liczby
 * obszarów przez żadnego z graczy.
 * @param[in] g       – wskaźnik na strukturę przechowującą stan gry,
 * @param[in] player  – numer gracza, liczba dodatnia niewiększa od wartości
 *                      @p players z funkcji @ref gamma_new.
...
/**
 * Funkcja wyświetlająca reprezentację planszy na standardowe wyjście.
 * Podświetla pole wskazywane przez argumenty podane w wywołaniu.
 * @param g         - wskaźnik na strukturę przechowującą stan gry. g != NULL.
 * @param x         - kolumna pola do wyróżnienia.
 * @param y         - wiersz pola do wyróżnienia.
 * @return Wielkość jednego pola w tkstowej reprezentacji planszy.
 * Dla gry z więcej niż 9 graczami, nie wilicza spacji między polami.
 */
uint32_t gamma_print_board(gamma_t *g, uint32_t x, uint32_t y);

/**
 * Zwraca parametr players z @p g.
 * @param g         - wskaźnik na strukturę przechowującą  grę.
 * @return Ilość graczy uczestniczących w grze.
 */
uint32_t gamma_get_players(gamma_t *g);

/**
 * Getter do parametru width w strukturze gamma_t do gry w trybie interaktywnym.
 * @param g         - wskaźnik na strukturę przechowującą planszę.
 * @return Ilość kolumn na planszy.
 */
uint32_t gamma_get_width(gamma_t *g);

/**
 * Getter do parametru height w strukturze gamma_t do gry w trybie
 * interaktywnym.
 * @param g         - wskaźnik na strukturę przechowującą planszę.
 * @return Ilość wirszy na planszy.
 */
